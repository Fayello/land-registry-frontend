...
export default function ClerkLayout({
    ...
    useEffect(() => {
        ...
        if (userStr) {
            ...
            const isAuthorizedClerk = ["clerk", "admin"].includes(role);

            if (!isAuthorizedClerk) {
                router.replace("/");
                setIsAuthorized(false);
                return;
            }
    ...
    const navItems = [
        { label: "Console Home", href: "/portal/clerk", icon: LayoutDashboard },
        { label: "Digitize Archives", href: "/portal/clerk/digitize", icon: Upload },
        { label: "Ingestion History", href: "/portal/clerk/history", icon: History },
    ];

    return (
        <div className="flex min-h-screen bg-slate-50 font-sans selection:bg-blue-500/20">
            {/* Sidebar */}
            <aside className="w-72 bg-white border-r border-slate-200 flex flex-col fixed h-full z-30 shadow-2xl shadow-slate-200/50">
                <div className="p-8 border-b border-slate-100">
                    <div className="flex items-center gap-3 mb-1">
                        ...
                        <h2 className="text-xl font-black text-slate-900 tracking-tighter">MINDAF<span className="text-blue-600">.gov</span></h2>
                    </div>
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] ml-1">Clerk Console</p>
                </div>
                ...
                <div className="flex-1 px-4 py-8 space-y-2 overflow-y-auto">
                    {navItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <item.icon size={20} className={isActive ? "text-blue-400" : "text-slate-400 group-hover:text-blue-600 transition-colors"} />
                ...
                <div className="p-6 border-t border-slate-100">
                    <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 flex items-center gap-3 mb-6">
                        ...
                        <div className="overflow-hidden">
                            ...
                            <p className="text-[10px] text-blue-600 font-black uppercase tracking-widest">Archive Officer</p>
                        </div>
                    </div>
